...
def load_teams(request):
    try:
        my_teams = Team.objects.filter(admin_id=request.user)
        teams = Team.objects.exclude(admin_id=request.user).order_by("rank").all()
...
def create_new_team(request):
    if request.method == "POST":
        ...
        try:
            ...
            print(new_team_id, "$$$$$$$$$$$$$$$$$$$$")  # kjsfkldflk klfdslk
            while (True):
...
def apply_team(request, team_id):
    ...
    notif = Notification(id=notif_id, sender=request.user, receiver=Team.objects.get(
        team_id=team_id).admin.username, message=message)
    notif.save()
